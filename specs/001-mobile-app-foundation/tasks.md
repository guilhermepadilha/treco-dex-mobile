...
- [x] T014 [US1] Implement automated local session recovery (JWT check from MMKV storage) on app startup in `/src/app/_layout.tsx`
...
- [x] T017 [P] [US2] Create object detail modal/screen in `/src/app/object/[id].tsx`
- [x] T018 [US2] Implement TanStack Query fetchers and custom hooks for fetching object lists and habitats in `/src/services/queries/useObjects.ts`
...
- [x] T020 [US2] Implement object physical state modification (e.g. updating item state/location) in `/src/app/object/[id].tsx` with offline fallback to useSyncStore
